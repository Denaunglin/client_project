...
@endsection
@section('expense-type-active','mm-active')
...
<script>
    $(function() {
            var table = $('.data-table').DataTable({
                ...
                ajax: {
                    'url' : '{{ url("/admin/expense_types?trash=0") }}',
                    'type': 'GET',
                },
                columns: [{
                        data: "plus-icon",
        ...
        $(document).on('change', '.trashswitch', function () {
            ...
            table.ajax.url(`{{url('/admin/expense_types?trash=`+trash+`/')}}`).load();
        ...
        $(document).on('click', '.trash', function (e) {
            ...
            swal("Are you sure, you want to trash?", {
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            url :`{{url('/admin/expense_types/`+id+`/trash')}}`,    
                            type: 'GET',
                            success: function () {
                                table.ajax.reload();
                            }
        ...
        $(document).on('click', '.restore', function (e) {
            ...
            swal("Are you sure, you want to restore?", {
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            url :`{{url('/admin/expense_types/`+id+`/restore')}}`,  
                            type: 'GET',
                            success: function () {
                                table.ajax.reload();
                            }
