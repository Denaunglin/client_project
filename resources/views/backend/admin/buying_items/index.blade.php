...
@section('page_title')
@lang("message.header.buying_item")
@endsection
@section('merchandise-active','mm-active')
...
<div class="d-flex justify-content-end">
    ...
    @can('add_item')
    <a href="{{route('admin.buying_items.create')}}" title="Add Category" class="btn btn-primary action-btn"> @lang("message.header.add_buying_item")</a>
    @endcan
</div>
@endsection
    ...
    var route_model_name = "buying_items";
        ...
        $(function() {
            app_table = $('.data-table').DataTable({
                ...
                ajax: {
                    'url' : '{{ url("/admin/buying_items?trash=0") }}',
                    'type': 'GET',
                },
                columns: [
        ...
        $('.datepicker').on('apply.daterangepicker', function(ev, picker) {
            ...
            app_table.ajax.url(`{{url('/admin/buying_items?daterange=`+daterange+`&trash=`+trash+`/')}}`).load();
        });
        ...
        $(document).on('change', '.trashswitch', function () {
            ...
            app_table.ajax.url(`{{url('/admin/buying_items?trash=`+trash+`/')}}`).load();
        });
        ...
        $(document).on('click', '.trash', function (e) {
            ...
            swal("Are you sure, you want to trash?", {
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            url :`{{url('/admin/buying_items/`+id+`/trash')}}`,
                            type: 'GET',
                            success: function () {
        ...
        $(document).on('click', '.restore', function (e) {
            ...
            swal("Are you sure, you want to restore?", {
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            url :`{{url('/admin/buying_items/`+id+`/restore')}}`,
                            type: 'GET',
                            success: function () {
                                app_table.ajax.reload();
                            }
